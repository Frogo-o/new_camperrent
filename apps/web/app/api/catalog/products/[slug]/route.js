import { NextResponse } from "next/server";
import { CATALOG_PRODUCT_DETAIL_TAG, CATALOG_PRODUCTS_TAG, catalogProductTag } from "../../../../../lib/catalog-cache";

const API_BASE_URL = (process.env.API_BASE_URL || "http://localhost:4000").replace(/\/+$/, "");
const PUBLIC_CACHE_CONTROL = "public, max-age=60, s-maxage=600, stale-while-revalidate=86400";

export const revalidate = 600;

export async function GET(req, ctx) {
  const { slug } = await ctx.params; // <- ключовото

  const target = `${API_BASE_URL}/api/catalog/products/${encodeURIComponent(String(slug))}`;

  const res = await fetch(target, {
    headers: { accept: "application/json" },
    next: { revalidate: 600, tags: [CATALOG_PRODUCTS_TAG, CATALOG_PRODUCT_DETAIL_TAG, catalogProductTag(slug)] },
  });

  const bodyText = await res.text();

  const nextRes = new NextResponse(bodyText, {
    status: res.status,
    headers: {
      "content-type": res.headers.get("content-type") || "application/json; charset=utf-8",
      "cache-control": res.ok ? PUBLIC_CACHE_CONTROL : "no-store",
    },
  });

  return nextRes;
}
